        ...
        if (!level.isClientSide()) {
            ...
            if (restored <= 0.0D) {
                player.sendSystemMessage(Component.translatable(
                        "message.fantasy_earth.mana_consumable.full",
            ...
            player.sendSystemMessage(Component.translatable("message.fantasy_earth.mana_consumable.used",
                    displayName,
                    ArcanaPower.format(restored),
                    ArcanaPower.format(ArcanaPower.getCurrentMana(player)),
                    ArcanaPower.format(ArcanaPower.getMaxMana(player))).withStyle(ChatFormatting.LIGHT_PURPLE));
            if (report.changed()) {
                player.sendSystemMessage(Component.translatable("message.fantasy_earth.human_recovery",

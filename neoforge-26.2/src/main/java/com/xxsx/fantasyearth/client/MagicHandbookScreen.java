...
import com.xxsx.fantasyearth.MagicJourney;
...
public class MagicHandbookScreen extends Screen {
    ...
    public MagicHandbookScreen() {
        super(Component.translatable("screen.fantasy_earth.handbook.title"));
    }
    ...
    protected void init() {
        ...
        prevButton = addRenderableWidget(Button.builder(Component.translatable("screen.fantasy_earth.handbook.prev"), b -> setPage(page - 1))
                .bounds(left + bookWidth() - 182, bottom, 76, 20).build());
        nextButton = addRenderableWidget(Button.builder(Component.translatable("screen.fantasy_earth.handbook.next"), b -> setPage(page + 1))
                .bounds(left + bookWidth() - 100, bottom, 76, 20).build());
        addRenderableWidget(Button.builder(Component.translatable("screen.fantasy_earth.handbook.close"), b -> onClose())
    ...
    public void extractRenderState(GuiGraphicsExtractor g, int mouseX, int mouseY, float delta) {
        ...
        String bookTitle = tr("screen.fantasy_earth.handbook.title");
        g.text(font, bookTitle, left + (bw - font.width(bookTitle)) / 2, top + 8, 0xFFE9E5F0, false);
        g.text(font, tr("screen.fantasy_earth.handbook.subtitle"), left + 14, top + 24, MUTED, false);
        g.text(font, tr("screen.fantasy_earth.handbook.page", page + 1, pages.size()), left + bw - 70, top + 24, MUTED, false);
    ...
    private List<Entry> liveStatusEntries() {
        ...
        entries.add(new Entry(Component.translatable(
                "screen.fantasy_earth.handbook.journey.progress",
        ...
        if (nextMilestone == null) {
            entries.add(new Entry(Component.translatable(
                    "screen.fantasy_earth.handbook.journey.complete").getString(), 10, GOLD));
        } else {
            entries.add(new Entry(Component.translatable(
                    "screen.fantasy_earth.handbook.journey.next",
        ...
        if (!status.isUnlocked(ArcaneFocus.ATTUNEMENT)) {
            entries.add(new Entry(Component.translatable(
                    "screen.fantasy_earth.handbook.status.locked").getString(), 10, PURPLE));
        ...
        String nextKey = unlocked < ArcaneFocus.values().length
                ? "screen.fantasy_earth.handbook.status.next.unlock"
                : "screen.fantasy_earth.handbook.status.next.train";
        entries.add(new Entry(Component.translatable(
                "screen.fantasy_earth.handbook.status.current",
    ...
    private static List<Page> createPages() {
        ...
        boolean xuanhuanLoaded = ModList.get().isLoaded("xuanhuan_earth")
                || ModList.get().isLoaded("earth_online_xuanhuan");
        if (!Minecraft.getInstance().getLanguageManager().getSelected().toLowerCase(Locale.ROOT).startsWith("zh")) {
            return List.of(
                    ...
                    page("Links", "5. Optional integrations", PURPLE,
                            ...
                            xuanhuanLoaded
                                    ? "Xuanhuan Earth connected: magic and qi contributions are added into one shared mana pool."
                                    : "Xuanhuan Earth not installed: this magic route remains fully standalone."));
        }
        return List.of(

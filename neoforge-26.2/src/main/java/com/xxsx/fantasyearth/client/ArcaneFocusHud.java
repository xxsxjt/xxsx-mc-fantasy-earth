...
public final class ArcaneFocusHud {
    private static final String[] STAGES = {
            "hud.fantasy_earth.focus.stage.stabilize",
            "hud.fantasy_earth.focus.stage.sense",
            "hud.fantasy_earth.focus.stage.attune",
            "hud.fantasy_earth.focus.stage.anchor"
    };
    ...
    public static void render(GuiGraphicsExtractor graphics, DeltaTracker deltaTracker) {
        ...
        Component focus = Component.translatable(
                com.xxsx.fantasyearth.ArcaneFocus.byId(status.focusId()).titleKey());
        Component title = fit(font, Component.translatable(
                "hud.fantasy_earth.focus.title", focus, status.focusLevel()), textWidth);
        graphics.text(font, title, textX, y + 7, 0xFFF8F4FF, false);

        Component stageText = Component.translatable(stageKey(stage));
        Component remainingText = Component.translatable("hud.fantasy_earth.focus.remaining", remainingSeconds);
        ...
        graphics.centeredText(font, fit(font, Component.translatable("hud.fantasy_earth.focus.hint"), panelWidth - 18),
